...
from main_network import MMCSE_Net
...
def train(args, model, train_loader, optimizer, criterion, epoch, device):
    ...
    for batch_idx, batch_data in enumerate(train_loader):
        ...
        loss = loss_valor_a + loss_valor_v + loss_video + loss_rec + loss_ort * 0.1 + loss_ec * 0.1
...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='Official Implementation of MMCSE')
    parser.add_argument("--audio_dir", type=str, default='../data/CLAP/features',
                        help="audio features dir")
    parser.add_argument("--video_dir", type=str, default='../data/CLIP/features',
                        help="2D visual features dir")
    parser.add_argument("--st_dir", type=str, default='../data/r2plus1d_18',
    ...
    model = MMCSE_Net(args).to(device)
